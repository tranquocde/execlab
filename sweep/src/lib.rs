...
pub type SweepFn = Box<dyn Fn(&[PathBuf], &str, &Path, &Path, usize, bool)>;
...
pub fn entry<A: alpha::Alpha + 'static>(name: &'static str, code_hash: &'static str) -> Entry {
    Entry {
        name,
        code_hash,
        total_space: A::search_space().len(),
        sweep: Box::new(move |files, data_dir, data_root, out, batch, force| {
            sweep::sweep::<A, _>(
                name,
                code_hash,
                files,
                ...
                data_root,
                out,
                population::Grid::new(A::search_space(), batch), // <- later: Ga::new(..)
                force,
            )
        }),
